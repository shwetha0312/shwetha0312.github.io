...
- Cosine similarity between hidden states of the token preceding the occurrence and the token preceding the last generated token (*x<sub>t-2</sub>*). (At time step t, we have access to hidden states of tokens only till *x<sub>t-2</sub>*, therefore we compare this hidden state with hidden state of the token preceding the occurrence)

- Attention map from “induction heads” computed for token *x<sub>t-1</sub>*
